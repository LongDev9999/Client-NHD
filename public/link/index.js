...
$(() => {
  ...
  function parseDataToTable(data = []) {
    return data.map((r) => {
      const { tenmon, batdau_dknhd, nguoi_thuchien, nguoi_nghiemthu,han_dknhd } = r;
      const payload = `${jsrmvi.removeVI(tenmon)}|${jsrmvi.removeVI(admin)}`;
      return [tenmon, admin, parseInt(lat, 10), parseInt(lng, 10), payload];
    });
  }
